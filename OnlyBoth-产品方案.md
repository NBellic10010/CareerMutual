...
**首个可运行主链：** Senior Backend Engineer；另以二十个跨领域合成岗位验证机制通用性，并以六个
技术 Match Lab 岗位比较六名工程向 Candidate 的 Candidate-only Eligibility Feed。
...
发布不可变 Snapshot，并在岗位 Feed 查看 GPT 生成的 evidence-linked Eligibility guidance。岗位 Feed
只呈现有效正向 Evidence match、`OPEN_TO_ALL` 与已有 Active Journey；不再提供绕过访问控制的
`Explore all jobs`。没有 Passport 的 Candidate 只能看到 `OPEN_TO_ALL`，AI 失败保持 Pending/Failed
而不是 Candidate 不合格。这些信号不进入 Employer Projection、Interest Queue 排序、Attention
...
初始合成目录包括一个支付可靠性工程岗、二十个跨领域岗位与六个技术 Match Lab 岗位，覆盖会计、FP&A、BD、Enterprise
...
1. 可选维护 Candidate-only Evidence Passport；Feed 聚合有效正向 Evidence match、`OPEN_TO_ALL` 与
   已有 Active Journey。不填写 Passport 时只可见 `OPEN_TO_ALL`，模型失败不会形成不合格结论；
2. 轻量表达岗位 Interest 与硬条件；Candidate Claim 和 Passport 不会进入需求方的回答前选择界面；
3. 不写长 Cover Letter，不支付 Bid；
4. 系统把 `OPEN_TO_ALL | AI_POSITIVE_EVIDENCE` 背景访问 pin 与显式法律/语言/时区硬条件共同写入
   `EligibilityEdge@2`；GPT 不参与硬条件判断；
...
Candidate 侧 Eligibility 不改变 Employer 盲审规则：Passport Match 只回到该 Candidate 的 Job Feed。
普通岗位必须有至少一条服务端验证的正向 Evidence-to-tag 连接；`OPEN_TO_ALL` 不要求 Passport。
未授权岗位统一不进入 Feed、详情或 Interest API。Employer 与 Scheduler 不读取 Passport、Match 或
connection；Scheduler 只读取最终 opaque Eligibility Edge，因此这不是 Employer pre-answer MatchEdge。
...
- 记录仍未知的部分；
- 为 Replay 生成可读解释。
- 从 Candidate-only Evidence Passport Snapshot 生成岗位 Eligibility Match、source refs 与
  `still_unknown`；只有服务端验证的正向连接可授权 Candidate 访问 evidence-gated 岗位；

### 12.2 确定性程序负责

- Contract Schema、版本、Seal 与 hash；
- Label Policy 的结构化字段隐藏；
- 法律/语言/时区硬资格、Eligibility Edge 的 deterministic 部分与非 Profile Answer Invitation；
...
- 输出 Candidate score/rank，改变队列顺序、Attention 分配或生成 Employer 候选人列表；
