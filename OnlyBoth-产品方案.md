...
发布不可变 Snapshot，并在岗位 Feed 查看 GPT 生成的 evidence-linked discovery guidance。岗位 Feed
默认进入 `Matched for you`，呈现有界连接与进行中的 Candidate Journey；`Explore all jobs` 继续提供
全部开放岗位，因此 discovery 缺失不会取消访问或 Interest 权利。这些信号不进入 Employer Projection、Eligibility、Interest Queue、Attention
...
1. 可选维护 Candidate-only Evidence Passport；默认 `Matched for you` 聚合带来源连接的岗位，`Explore all jobs` 保持全部开放岗位可访问，不填写 Passport 也不会失去岗位访问权；
...
Candidate 侧岗位发现不改变这条规则：Passport Signal 只回到该 Candidate 的 Job Feed。默认
`Matched for you` 使用连接信号减少首屏噪音，次级 `Explore all jobs` 保持所有开放岗位可访问，
Candidate 自己决定是否登记 Interest。Employer、Eligibility 与 Scheduler 不读取
