...
require_text "AGENTS.md" 'A Candidate without a published Passport sees only `OPEN_TO_ALL` roles.' "AGENTS makes evidence-gated role access fail closed"
require_text "AGENTS.md" '`deriveCandidateEligibilityMatches` is the only AI operation with Candidate Feed access authority.' "AGENTS assigns Feed authority only to validated Eligibility matches"
