...
六名合成 Candidate 各有独立的 Candidate-only Evidence Passport discovery state。它允许 Candidate
发布合成来源 Snapshot，并用 `deriveCandidateJobSignals` 将 source refs 与公开 Job capability
建立发现假设；它不属于 Employer Matching，不改变 Eligibility、Queue、Invitation 或 Attention。
...
- Candidate Assistant 只生成披露式草稿建议；原始 Voice Memo 与派生 Transcript 分离；二者
  都没有业务写权限或最终提交权限。
- Candidate discovery 只帮助 Candidate 浏览全部开放岗位；Signal 不是 verified capability、
  Fit Score 或 Employer 输入，Candidate 仍亲自决定是否登记 Interest。
...
## 5. 四个 Hiring Operation 与一个 Candidate discovery Operation 的契约
...
没有任何 Candidate matching AI request；回答后也只从 Answer Evidence Repository 组装 opaque
DTO，因此不是依靠 Prompt “要求模型忽略姓名或履历”，而是在查询层根本无法取到这些字段。
...
Candidate Discovery Assembler 是第三条隔离依赖：composition root 只注入 Passport Snapshot
Repository 和公开 Job Contract Repository，不注入 Private Label、Employer Projection、Queue、
Eligibility 或 Attention Repository。它删除 display title、原始 locator，并只发送脱敏描述、
source hash 与 opaque refs。
...
Evidence Passport demo 只允许 seed 一份带 `synthetic_preloaded=true` 的 Candidate-only Signal
Snapshot，便于无 Key 浏览。Candidate 一旦编辑并发布，新的 `CandidateDiscoveryRequested` 必须
...
| Pre-answer Candidate selection | No Candidate matching AI request + no Profile/Claim repository in Employer assembler | Pre-answer payload and no-call test |
...
- Demo 只有一份明确 synthetic 的预载 Signal Snapshot；后续 Publish/Refresh 无 Key 时显式失败，
  且 Employer、Eligibility、Queue、Invitation 与 Attention stores 不读取 Passport/Signal 表；

- Backed Offer 前没有 Candidate matching AI request，Employer 输入边界没有 Profile、Claim、
  MatchEdge 或 résumé labels；
...
- 四个 Hiring Intelligence Operation 通过同一 Application Port 调用，Candidate Assistant 与
  Transcription 通过两个更小的 Worker-only Port 调用；
- 回答前不存在 Candidate matching AI request，Employer/AI payload 不含 Profile、Claim 或匹配 rationale；
...
- Candidate discovery 使用独立 Port、Candidate-only Projection、全岗位可见 Feed 和不可变
  Snapshot pins；其输出不能进入 Employer、Eligibility、Queue、Invitation 或 Attention；
