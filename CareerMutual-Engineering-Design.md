...
Media Asset fields are fixed as `asset_ref/source_kind/file_name/MIME/bytes/SHA-256/download_url`; only same-origin relative paths are accepted, and Seed is marked `SYNTHETIC_SEED`. TEXT cannot carry an Asset at the same time, and a media Part cannot carry `text_content` at the same time; AUDIO/IMAGE must pass MIME semantic validation, IMAGE must have alt text, and Part refs must be unique. At publication, the entire Manifest enters the Contract hash, and the Answer `question_hash` is also bound to the complete Manifest rather than only the old string. Therefore Candidate and Reviewer read the same order and content after refreshes and restarts.

Migration `0015_employer_challenge_assets` adds the Recruiter upload vertical without granting the browser direct object authority. The Employer Command creates a five-minute, create-only private PUT for an exact `IMAGE | AUDIO | FILE` MIME and byte count. The Composer gives a local preview, but serializes only a server-confirmed `VERIFIED` receipt. Complete reads private-object metadata and bytes, checks the declared size and MIME, SHA-256, and a deterministic content signature; image alt text and audio transcript excerpts are required accessibility metadata. Draft create/update reloads every owner-bound Asset from PostgreSQL and rejects browser-forged metadata, duplicate refs, cross-owner refs, non-verified objects, and media presented as a synthetic Seed. Only the local synthetic reset script receives the internal fixture-write capability needed to preserve disclosed Seed assets; authenticated Web command contexts cannot set or deserialize that capability.

Publish locks the Draft and all referenced Asset rows in the same PostgreSQL transaction, writes the sealed Contract hash, and changes each `VERIFIED` Asset to immutable `SEALED` with the new Opportunity ref. Recruiter reads require the owning Employer Session. Candidate reads require the same Feed authorization as the JobPost (`OPEN_TO_ALL`, a current validated positive Eligibility match, or an already active journey), and all reads are private, `no-store`, and `nosniff`. Raw object keys and public bucket access are never returned. `VIDEO` is absent from the part/upload enums and MIME allowlist; the Composer exposes only a disabled `Video — later` marker until a separate video inspection policy exists.
