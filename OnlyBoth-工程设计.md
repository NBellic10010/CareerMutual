...
Evidence Passport 是单独的 Candidate-owned 数据边界。Candidate Eligibility Assembler 只能读取
...
没有 Employer Projection、Interest Queue、Invitation 或 Attention 写端口。它只能完成一个
Candidate-only Match Set；Feed/Detail/Interest 重新读取当前版本的正向 Match 或 OPEN policy。
...
| `CandidateEligibilityProjection` | Candidate-only `MATCHING | READY | PARTIAL | FAILED | STALE`、Passport pin；Feed 仅含 positive match、OPEN_TO_ALL 与 pinned active Journey |
...
Candidate 可以在 Stage 1 之前使用独立的 Candidate-side Eligibility Match；它不属于 Employer
Matching Pipeline：
...
普通岗位必须至少有一条正向连接才能进入 Feed；`OPEN_TO_ALL` 不要求 Passport。无 Passport 时只
显示 OPEN；AI failure 保持 Pending/Failed，不能生成不合格结论。该支路不输出 Fit Score、不向
Employer 暴露 connection，也不连接 Queue Scheduler、Slot 或 Allocation。
`SYNTHETIC_SOURCE_ATTACHED` 不能被模型提升为 verified capability。
...
- 背景访问由 `OPEN_TO_ALL | AI_POSITIVE_EVIDENCE` 建立；法律、语言、时区 hard Eligibility 与
  Answer Invitation Queue Scheduling 仍为确定性、非 Profile 约束；
...
它使用 `gpt-5.6-sol`、`reasoning.effort=medium`、Responses Structured Outputs、`store:false`，
无 tools/conversation/background。Validator 要求每个输入 Job 恰好一项结果；Education 只能连接
Education tag，其他 Evidence 只能连接 Work Domain tag；禁止 score/rank/Hire/Reject/Queue/Attention。
...
- 压缩已有 Evidence；
- 明确输出未知或拒绝判断。
- 为 Candidate 自己生成基于来源引用的岗位 Eligibility Match；合法正向连接可解锁 evidence-gated
  岗位，OPEN_TO_ALL 始终可访问，但不得影响 Employer 路径或 Queue 顺序。
...
预载 Candidate-only Eligibility Match；Draft save、Snapshot publish、
Outbox `CandidateDiscoveryRequested`、Worker Responses Adapter、deterministic ref/policy validation 和
Candidate Feed V3。Candidate 42 的主 Backend 连接使用一次真实 Responses API 调用后保存、校验并
pin 的 `RECORDED_LIVE` 输出；其他预载 Match 明确为 synthetic。任何后续 Publish
将旧结果置为 stale 并请求 LIVE `gpt-5.6-luna`。缺少 Key 时 Worker 将 Signal Set 置为 FAILED，
不产生固定回放成功。Eligibility Match 则请求 `gpt-5.6-sol`，失败绝不切换该录制输出。
Employer Query、Queue 与 Attention stores 均不读取 Match 内容。
...
创建 1 个主工程岗、20 个跨领域岗位和 6 个技术 Match Lab 岗位，而不是直接预写 Projection。Match
Lab 只为六名工程向合成 Candidate 生成多对多 Candidate-only Eligibility Feed；它不进入 Employer
候选人列表、Queue 排序或 Attention 分配。
